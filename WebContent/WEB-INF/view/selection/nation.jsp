...
<head>
    ...
    <script type="text/javascript">
        ...
        var toolbar = [{
            ...
            handler: function () {
                var buttons = [{
                    ...
                    text: '保存',
                    handler: function () {
                        var nationName = $.trim($('#addNation').find('#nationName').val());
                        var obj = {};
                        if (nationName.length > 0) {
                            obj.nationName = nationName;
                        } else {
                            $.messager.alert('提示', "请填写民族名称！");
                            return;
                        }
                        $.ajax({
                            url: $cntPath + "/nation.spr?method=addNation",
                            data: "nationName=" + JSON.stringify(obj),
                            method: "post",
                            dataType: "json",
                            async: false,
                ...
                $('#win').dialog({
                    ...
                    closed: false,
                    cache: false,
                    href: $cntPath + '/nation.spr?method=toAddNation',
                    modal: true,
                    buttons: buttons
                });
            }
            ...
            handler: function () {
                ...
                var buttons = [{
                    ...
                    text: '保存',
                    handler: function () {
                        var nationName = $.trim($("#modifyNation").find("#nationName").val());
                        if (nationName.length <= 0) {
                            $.messager.alert('提示', "请填写民族名称！");
                            return;
                        }
                        $.ajax({
                            url: $cntPath + "/nation.spr?method=updateNationName",
                            data: "nationName=" + nationName + "&id=" + id,
                            method: "post",
                            dataType: "json",
                            async: false,
                            success: function (data) {
                ...
                $('#win').dialog({
                    ...
                    closed: false,
                    cache: false,
                    href: $cntPath + '/nation.spr?method=toModifyNation',
                    modal: true,
                    buttons: buttons
                });
            ...
            handler: function () {
                var selectObj = $("#dg").datagrid('getSelected');
                if (selectObj == null) {
                    $.messager.alert('提示', "请选择要删除的民族分类！");
                    return;
                } else {

                    $.messager.confirm('确认', '确定要删除该民族分类？', function (r) {
                        if (r) {
                            $.ajax({
                                url: $cntPath + "/nation.spr?method=deleteNation",
        ...
        $(function () {
            ...
            grid = new DataGrid("nationQueryGrid", "#dg");
            grid.load({}, {toolbar: toolbar});
            setHeight("cc");
        });
        function query() {
