...
$(function() {
	...
	$.ajax({
		url:$cntPath+"/rkbl.spr?method=queryAreaAgeRatio",
		method:"post",
		async:false,
		success:function(tmpData){
			if(tmpData){
				for(var i=0;i<tmpData.length;i++){
					...
					data[0].value.push(tmpData[i]["AGE1"]);
					data[1].value.push(tmpData[i]["AGE2"]);
					data[2].value.push(tmpData[i]["AGE3"]);
				}
			}
		},
		error:function(msg){
