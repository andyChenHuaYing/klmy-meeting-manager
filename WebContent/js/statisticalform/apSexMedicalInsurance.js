...
$(function() {
	...
	$("#btnDiv").css("top",55);
	$("#btnDiv2").css("top",485);
	 var title="绥化参合人员按性别统计（柱形图）";
	 var title2="绥化参合人员按性别统计（饼图）";
	 var labels = [];
...
//	} ];
	$.ajax({
		url : $cntPath + "/chrybl.spr?method=querySexCoMedicalInsurance",
		method : "post",
		async : false,
		success : function(tmpData) {
			if (tmpData) {
				...
				for (var i = 0; i < tmpData.length; i++) {
					labels.push(tmpData[i]["SEX"]);
					data[0].value.push(tmpData[i]["NUM"]/totalCnt*100);
					
					var obj = {};
					obj.name = tmpData[i]["SEX"];
