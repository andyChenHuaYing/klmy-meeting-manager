...
$(function() {
	...
	$.ajax({
		url:$cntPath+"/nnbl.spr?method=querySexAgePeriodNumber",
		method:"post",
		async:false,
		success:function(tmpData){
			if(tmpData){
				if(tmpData.labelList){
					labels = tmpData.labelList;
				}
				if(tmpData.maleList){
					data[0].value = tmpData.maleList;
				}
				if(tmpData.femaleList){
					data[1].value = tmpData.femaleList;
				}
			}
		},
		error:function(msg){
